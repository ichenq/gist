...
#include "Uri.h"
#include <ctype.h>
#include "Utility/Portability.h"

// The GNU C++ standard library supports <regex>, but not until version 4.9.0
#if _MSC_VER >= 1800 || __GNUC_PREREQ(4, 9) || defined(__clang__)
#include <regex>
namespace cpplib = std;
#else
#include <boost/regex.hpp>
namespace cpplib = boost;
#endif
...
std::string submatch(const cpplib::cmatch& m, size_t idx)
{
    auto& sub = m[idx];
...
{
    static const cpplib::regex uriRegex(
        "([a-zA-Z][a-zA-Z0-9+.-]*):"  // scheme:
        "([^?#]*)"                    // authority and path
        "(?:\\?([^#]*))?"             // ?query
        "(?:#(.*))?");                // #fragment
    static const cpplib::regex authorityAndPathRegex("//([^/]*)(/.*)?");

    cpplib::cmatch match;
    if (UNLIKELY(!cpplib::regex_match(str.begin(), str.end(), match, uriRegex))) {
    ...
    cpplib::cmatch authorityAndPathMatch;
    if (!cpplib::regex_match(authorityAndPath.begin(),
        ...
        static const cpplib::regex authorityRegex(
            "(?:([^@:]*)(?::([^@]*))?@)?"  // username, password
        ...
        cpplib::cmatch authorityMatch;
        if (!cpplib::regex_match(authority.first,
            authority.second,
            authorityMatch,
            authorityRegex)) {
...
const std::vector<std::pair<std::string, std::string>>& Uri::getQueryParams()
{
    if (!query_.empty() && queryParams_.empty()) {
        // Parse query string
        static const cpplib::regex queryParamRegex(
            "(^|&)" /*start of query or start of parameter "&"*/
            "([^=&]*)=?" /*parameter name and "=" if value is expected*/
            "([^=&]*)" /*parameter value*/
            "(?=(&|$))" /*forward reference, next should be end of query or
                          start of next parameter*/);
        cpplib::cregex_iterator paramBeginItr(
            query_.data(), query_.data() + query_.size(), queryParamRegex);
        cpplib::cregex_iterator paramEndItr;
